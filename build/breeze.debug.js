...
(function (global, definition) {
    ...
    var breeze = {
		...
		selectClauseThrowIfNotFound: false
    };
    ;/**
 @module core
 **/
...
var CsdlMetadataParser = (function () {
    ...
    function parseCsdlEntityType(csdlEntityType, schema, metadataStore, onCreateModel) {
      ...
      var baseTypeName = parseTypeNameWithSchema(csdlEntityType.baseType, schema).typeName;
            ...
            if (baseEntityType) {
                completeParseCsdlEntityType(entityType, csdlEntityType, schema, metadataStore, onCreateModel);
            } else {
  ...
  function completeParseCsdlEntityType(entityType, csdlEntityType, schema, metadataStore, onCreateModel) {
        ...
        if (deferrals) {
            deferrals.forEach(function (d) {
                completeParseCsdlEntityType(d.entityType, d.csdlEntityType, schema, metadataStore, onCreateModel);
            });
...
var SelectClause = (function () {
    ...
    this.propertyPaths.forEach(function (path) {
            entityType.getProperty(path, breeze.selectClauseThrowIfNotFound);
        });
    };
...
//  }
  ...
  function markLoadedNavPath(entities, propNames) {
    ...
    function unwrapInstance(structObj, transformFn, options) {
         ...
         */
        if (options.ignoreNavigations !== true) {
  ...
  */
  ...
  breeze.config.registerAdapter("dataService", webApiOData4Ctor);
    ...
    proto.executeQuery = function (mappingContext) {
        ...
        /*
         * Send directly request for actions
         */
        if (mappingContext.query && mappingContext.query.parameters && Object.keys(mappingContext.query.parameters).length > 0) {
            OData.request({ requestUri: serviceUrl + methodUrl, method: "POST", data: mappingContext.query.parameters },
                ...
                if (response.statusCode == 200) {
                    ...
                    return deferred.resolve({ results: data, inlineCount: inlineCount, httpResponse: response });
                }
                return deferred.reject(createError({ response: response }, url));
            }, function (error) {
                return deferred.reject(createError(error, url));
        ...
        /*
         * Otherwise, send as batch request
         */
        OData.request({
            requestUri: serviceUrl + "/$batch",
            method: "POST",
                            ...
                            "dataServices":
 {
    ...
    function createChangeRequests(saveContext, saveBundle) {
        ...
        saveBundle.entities.forEach(function (entity, index) {
            ...
            if (aspect.entityState.isAdded()) {
                var options = { readedAssociations: readedAssociations, ignoreNavigations: true };
                insertRequest(request, entity.entityType, routePrefix);
                request.method = "POST";
                request.data = helper.unwrapInstance(entity, transformValue, options);
                tempKeys[id] = aspect.getKey();
    ...
    function insertRequest(request, entityType, routePrefix) {
        ...
        do {
            lastBaseType = sbaseType;
            sbaseType = sbaseType.baseEntityType;
        } while (sbaseType && !lastBaseType.forceResourceName);
...
;( function ( factory ) {
    if ( typeof breeze === "object" ) {
        ...
        proto.buildUri = function ( entityQuery, metadataStore ) {
            ...
            if ( !entityType.isAnonymous && entityQuery.resultEntityType && entityQuery.resultEntityType.baseEntityType ) {
                var namespace = entityQuery.resultEntityType.namespace;
                var className = entityQuery.resultEntityType.name.replace( ":#" + namespace, "" );
                return entityQuery.resourceName + "/" + namespace + "." + className + qoText;
            }
